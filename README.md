...
6. 选中多声道曲目后，播放器会显示“多声道试听”。“全部声道”保留原始混音；点击 FL/FR/FC/LFE/SL/SR 等按钮可独听一个声道，并复制到左右前声道，适合用耳机检查各声道内容。
...
- 支持 SCD v3 中的 OGG 包装版本 2 / 3。浏览器解码为 PCM，AudioWorklet 按采样点循环；44.1 kHz 到 48 kHz 的重采样会同步换算循环位置。多声道默认保持原始声道数量，独听模式把目标声道复制到前左/前右并静音其余输出。
- HCA 曲目显示明确的暂不支持提示；无音轨占位条目也会说明原因。未选择或缺失的资料片音乐禁用，不会误报为可播放。
- 当前一次解码一首曲目，不是流式 PCM 解码；估算 PCM 大于 320 MiB 时提示超出内存限制。切歌会释放上一首音频。多声道按浏览器扬声器规则输出；声道独听用于分析，不等于游戏内变体切换。
