...
2. 首次打开时在“设置”里的“资源目录”点击“重新选择”，选择游戏的 `game/sqpack`；会读取 `ffxiv` 和 `ex1`～`ex5` 等已存在的资料片目录。也支持只选 `game/sqpack/ffxiv`。
...
首次打开时选择目录并申请只读权限；之后会在浏览器本地记住目录句柄，权限仍有效时自动恢复，权限需要重新确认时点击“恢复上次目录”即可。只保存循环、音量和播放顺序偏好，不保存播放位置。曲库、音乐读取、解包和解码均在本地，没有上传接口，不复制整个游戏目录。
