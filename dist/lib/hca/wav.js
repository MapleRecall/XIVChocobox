 ...
 */
export function encodeToWav(channelCount, samplingRate, pcm, options = {}) {
    ...
    if (pcm.length % channelCount !== 0) {
        throw new RangeError(`pcm length (${pcm.length}) is not a multiple of channelCount (${channelCount})`);
    }
    ...
    switch (bitDepth) {
        ...
        default: {
            throw new RangeError(`Unsupported bitDepth: ${bitDepth} (must be 0, 8, 16, 24, or 32)`);
        }
    }
    return out;
}
